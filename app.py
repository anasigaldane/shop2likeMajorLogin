...
# === Concurrency control ===
MAX_CONCURRENT_REQUESTS = 500  # يمكن زيادة العدد حسب قدرات Vercel
...
# === Token Generation with retry and lock ===
...
async def create_jwt(region: str):
    async with token_locks[region]:  # lock لمنع سباق إنشاء التوكن
        ...
        for attempt in range(retries):
            try:
                ...
                logger.warning(f"JWT creation failed ({attempt+1}/{retries}) for {region}: {e}")
                await asyncio.sleep(1)
        raise RuntimeError(f"Failed to create JWT for {region} after retries")
...
# === Fetch Account Information with semaphore ===
    ...
    async with semaphore:
        ...
        for attempt in range(retries):
            try:
                ...
                logger.warning(f"Fetch account info failed ({attempt+1}/{retries}): {e}")
                await asyncio.sleep(1)
        raise RuntimeError("Failed to fetch account info after retries")
